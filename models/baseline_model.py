...
class PhysicsPDECell(nn.Module):
    ...
    def __init__(self):
        ...
        # Fused PDE filters: stack Sobel-X, Sobel-Y, Laplacian into single (3,1,3,3) tensor
        # This computes all 3 spatial derivatives in ONE conv2d call (saves 32 kernel launches/sample)
        sobel_x = torch.tensor([[-1., 0., 1.], [-2., 0., 2.], [-1., 0., 1.]]) / 8.0
        sobel_y = torch.tensor([[-1., -2., -1.], [0., 0., 0.], [1., 2., 1.]]) / 8.0
        laplacian = torch.tensor([[0., 1., 0.], [1., -4., 1.], [0., 1., 0.]])
        pde_filters = torch.stack([sobel_x, sobel_y, laplacian]).unsqueeze(1)
        self.register_buffer('pde_filters', pde_filters)
    ...
    def _step(self, c, u, v, pblh, rain_mask, emissions):
        ...
        """
        # Single fused conv2d → 3 output channels: [dC/dx, dC/dy, ∇²C]
        spatial_derivs = F.conv2d(c, self.pde_filters, padding=1)
        dc_dx = spatial_derivs[:, 0:1, :, :]
        dc_dy = spatial_derivs[:, 1:2, :, :]
        lapl_c = spatial_derivs[:, 2:3, :, :]
...
class FNO2D(nn.Module):
    ...
    def forward(self, x):
        ...
        # ========================================
        # Pre-allocate output tensor (avoids dynamic list append + torch.cat)
        out_tensor = torch.empty((b, self.time_out, nx, ny), device=x.device, dtype=x.dtype)
        ...
        for h in range(self.time_out):
            ...
            out_tensor[:, h:h+1, :, :] = c_next                # direct insert
            c = c_next
