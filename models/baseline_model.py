...
class FNO2D(nn.Module):
    ...
    def __init__(self, in_channels, time_out=16, width=64, modes=None,
        ...
        # Flatten time into channels: C_in * T_in
        flat_channels = in_channels * time_input
        base_width = width  # Configurable via train.yaml
        ...
        self.conv_up1 = DoubleConv(base_width * 8, base_width * 4)  # *8 from skip concatenation
    ...
    def forward(self, x):
        ...
        # Only use first T_in hours — avoids artificial gradient cliff
        ...
        x1 = self.inc(x_flat)       # (B, W, H, W)
        x2 = self.down1(x1)         # (B, 2W, H/2, W/2)
        x3 = self.down2(x2)         # (B, 4W, H/4, W/4)
        x4 = self.down3(x3)         # (B, 8W, H/8, W/8)
        ...
        out = self.outc(x_up3)  # (B, 16, H, W)
        
        # Residual: anchor to last known PM2.5
        out = out + last_pm25.unsqueeze(1)
