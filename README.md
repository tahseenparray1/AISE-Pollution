...
The project has 6 scripts inside `scripts/`. The first three form the main pipeline (run in order), and the remaining three are for analysis and visualisation.

### Main Pipeline

| Script | What it does |
|---|---|
| `prepare_dataset.py` | Loads raw `.npy` data files for each month, computes derived weather features (wind speed, ventilation coefficient, rain mask), normalises all variables, and saves the processed training data as ready-to-use arrays. |
| `train.py` | Trains 3 copies of the WNO model with different random seeds. Each model trains on the prepared data using a combined loss (MSE + spatial gradient penalty). After training, Stochastic Weight Averaging (SWA) is applied. Saves one checkpoint per seed. |
| `infer.py` | Loads all 3 trained model checkpoints one by one, runs each on the test data, averages their predictions (ensemble), converts back to physical PM2.5 units, clips negatives to zero, and saves the final `preds.npy`. |

### Analysis & Visualisation

| Script | What it does |
|---|---|
| `data_exploration.py` | Generates exploratory plots — seasonal PM2.5 maps across months, emission source hotspot maps, and a time-series showing PM2.5 vs boundary layer height at the most polluted grid cell. |
| `results_analysis.py` | Compares model predictions against ground truth — plots RMSE per forecast hour, a spatial error map showing where the model struggles most, and predicted vs actual PM2.5 curves. Also prints the overall test RMSE. |
| `interpret_results.py` | Produces a detailed analysis of `preds.npy` — summary statistics, spatial heatmaps at different forecast hours, PM2.5 distribution histograms, hotspot/coldspot maps, anomaly maps, and hour-to-hour change rates. Can optionally compare against ground truth if provided. |
...
| Package | Purpose |
|---|---|
| `torch>=2.0` | Deep learning framework (training, inference, GPU support) |
...
Link : https://www.kaggle.com/code/tahseen123/group-id-46-prml-project

and run the code cells in order.

## Notes

- The full pipeline takes approximately **3-4 hours** on a Kaggle P100 GPU.
- `/kaggle/working/` is persistent storage (up to 20 GB) — final outputs are saved here.
- `/kaggle/temp/` is non-persistent — used for large intermediate data.
