...
for ep in range(cfg.training.epochs):
    ...
    for x, y in tqdm(train_loader, desc=f"Epoch {ep}"):
        ...
        with torch.amp.autocast('cuda'):
            ...
            # 1. Get physical targets (no gradients needed — y is not model output)
            with torch.no_grad():
                targ_phys = to_physical(y)
            
            # 2. Physical Weight Map: spikes get proportionally more gradient.
            #    Background (<30 µg/m³) gets weight 1.0.
            #    A spike of 300 gets weight 10.0. A spike of 600 gets 20.0.
            weights = torch.clamp(targ_phys / 30.0, min=1.0)
            
            # 3. Unreduced latent MSE, weighted by physical importance
            raw_latent_mse = F.mse_loss(out, y, reduction='none')
            total_loss = (raw_latent_mse * weights).mean()

        # Convert to physical space ONLY for diagnostic printouts
        with torch.no_grad():
        ...
        # Backpropagate the physically-weighted latent loss
    ...
    print(f"  Train RMSE: {train_rmse:.4f} | Val RMSE: {val_rmse:.4f} | Weighted Loss: {avg_latent_loss:.6f}")
    ...
    epoch_log = {
        ...
        "weighted_loss": round(avg_latent_loss, 6),
