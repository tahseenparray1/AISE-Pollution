...
def spatial_weighted_mse_loss(pred_norm, targ_norm, pred_phys, targ_phys):
    ...
    # This focuses the model on the most difficult region without panicking over single-pixel spikes.
    weights[:, 60:90, 40:100, :] *= 3.0

    # 3. Safe Normalization (Weights must sum to 1)
...
for ep in range(cfg.training.epochs):
    ...
    for step, (x, y) in enumerate(tqdm(train_loader, desc=f"Epoch {ep}")):
        ...
        # --- OPTION A: THE STABLE SPATIAL MSE ---
        spatial_mse_loss = spatial_weighted_mse_loss(out_f32, y_f32, pred_phys, targ_phys)
        loss_grad = spatial_gradient_loss(out_f32, y_f32)
        
        total_loss = spatial_mse_loss + 0.1 * loss_grad
        ...
        if step % 20 == 0:
            ...
            log_msg = (
                ...
                f"SpatialMSE: {spatial_mse_loss.item():.4f} | "
    ...
    val_spatial_mse_acc, val_grad_acc = 0.0, 0.0
    ...
    with torch.no_grad():
        for x, y in val_loader:
            ...
            val_spatial_mse_acc += spatial_weighted_mse_loss(out_f32, y_f32, pred_phys, targ_phys).item()
    ...
    logging.info(f" [Loss] Val SpatialMSELoss: {(val_spatial_mse_acc / len(val_loader)):.4f} | Val Grad: {(val_grad_acc / len(val_loader)):.4f}")
    logging.info("-------------------------------------")
